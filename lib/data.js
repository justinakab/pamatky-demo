export const monuments = [
  {
    id: 'rudolfinum',
    name: 'Rudolfinum',
    description:
      'Rudolfinum je novorenesanční kulturní budova na náměstí Jana Palacha při pravém břehu řeky Vltavy na Starém Městě v Praze. V její Dvořákově síni se pořádají významné koncerty, např. v rámci Pražského jara. Galerie Rudolfinum slouží k výstavám výtvarného umění. Budova Rudolfina patří České filharmonii, která zde má zároveň své hlavní sídlo.',
    image: 'http://localhost:4000/assets/',
  },
  {
    id: 'orloj',
    name: 'Staroměstský Orloj',
    description:
      'Staroměstský orloj nebo také Pražský orloj jsou středověké astronomické hodiny umístěné na jižní straně věže Staroměstské radnice v Praze. Orloj je poprvé historicky doložen v listině z 9. října 1410 dochované v opisu.',
    image: 'http://localhost:4000/assets/',
  },
  {
    id: 'narodniDivadlo',
    name: 'Národní Divadlo',
    description: 'o',
    image: 'adresa',
  },
  {
    id: 'tanciciDum',
    name: 'Tančící Dům',
    description: 'o',
    image: 'adresa',
  },
];
